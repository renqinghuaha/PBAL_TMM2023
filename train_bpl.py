...
def train(cfg, writer, logger):
    ...
    for epoch in range(epoches):
        ...
        for target_batch in datasets.target_train_loader:
            ...
            target_image = target_batch['img'].to(device)
            ...
            loss_GTA, loss_adv_G, loss_D, loss_contrast = model.step_bpl(source_image, source_label, target_image, adv_usage=True)

            time_meter.update(time.time() - start_ts)
            if (i + 1) % cfg['training']['print_interval'] == 0:
                fmt_str = "Epoches [{:d}/{:d}] Iter [{:d}/{:d}]  loss_GTA: {:.4f}  loss_adv_G: {:.4f}  loss_D: {:.4f}  loss_contrast: {:.4f} Time/Image: {:.4f}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default='configs/stage1_BPL.yml',
